...
for idx = 1:length(lambda)
    for idx_alpha= 1:length(alpha)
        alpha_tmp = alpha(idx_alpha);
        k_tmp = k(idx);
        hold on
        k_s_x = k_tmp.*sin(theta)-k_tmp*sin(alpha_tmp);
        k_s_y = -k_tmp.*cos(theta)+k_tmp*cos(alpha_tmp);
...
set(gcf,'color','w')
exportgraphics(fig,'mWavelength_mAngle.png','Resolution',300)
exportgraphics(fig,'mWavelength_mAngle.eps')