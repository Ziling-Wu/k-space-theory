...
hold on
gama_arc = linspace(3*pi/2,3*pi/2+theta_0,  20);
c = circle_arc([0, 0], gama_arc,4);
plot(c(1,:), c(2,:), 'color',[0.5 0.5 0.5])
hold on
text(c(1,10)+0.3 ,c(2,10)-3, '$\theta$', 'interpreter', 'latex', 'verticalalignment', ...
...
text((k-3)*sin(theta_0),-k*cos(theta_0)+5, '$k_{d}$', 'interpreter', 'latex', 'horizontalalignment', 'left')
% plot sample wave vector:
arrow([0, -k], [k*sin(theta_0), -k*cos(theta_0)], 'length', 4,'tipangle', 35, 'width', .5, 'color', [255 0 0]/255)
text((k)*sin(theta_0-pi/9)+1,-k*cos(theta_0-pi/9)-1, '$k_{s}$', 'interpreter', 'latex', 'horizontalalignment', 'left')
hold on
...
hold on
plot(k_s_x,k_s_y,'LineWidth',1.5,'color','r');
grid on
% plot axis vectors:
...
% tightfig;
set(gcf,'color','w')
exportgraphics(fig,'TransHolo.png','Resolution',300)
exportgraphics(fig,'TransHolo.eps')